 ...
 *
 * No NestJS, no Prisma, no HTTP — this is the domain layer, and it is the
 * authoritative version. The frontend carries a byte-identical copy in
 * `frontend/src/lib/pricing.ts` so it can render prices without a round trip,
 * but a price arriving from a browser is never trusted: every order is priced
 * here, server-side, from rows read inside the placing transaction.
 *
 * ── The model ────────────────────────────────────────────────────────────────
 *
